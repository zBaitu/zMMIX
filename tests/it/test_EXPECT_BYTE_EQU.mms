...
FAILED_STR     BYTE "[FAILED]",0
    ...
    GREG @
    PREFIX :test_EXPECT_BYTE_EQU:
suite_name    BYTE " test_EXPECT_BYTE_EQU ",0
    PREFIX :test_EXPECT_BYTE_EQU:eq:
case_name    BYTE " eq ",0
left    BYTE 255
right   BYTE 255

    PREFIX :test_EXPECT_BYTE_EQU:ne:
case_name    BYTE " ne ",0
left    BYTE #ff
right   BYTE #fe
...
:PrintStrAssetionFail GET rj_bak,:rJ
    ...
    PREFIX :EXPECT_BYTE_EQU
value_addr          IS $0
expected_value_addr IS $1
MAX_NUM_STR_LEN     IS 3

                    GREG @
ASSERTION_STR       BYTE "EXPECT_BYTE_EQU",#a,0
value               GREG
expected_value      GREG
result              GREG
rj_bak              GREG

:EXPECT_BYTE_EQU    LDBU    value,value_addr
